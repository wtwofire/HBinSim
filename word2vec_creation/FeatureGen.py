...
def cal_operand_embedding(insnToken_embeddings, insn_opcode):
    size = tf.to_float(tf.size(insnToken_embeddings)-1)
...
def buildAndTraining(articlelist, reversed_dictionarydir, deepbindiffinfodir):
    global data_index
    global random_walk_done
    global learning_rate
    with open(deepbindiffinfodir, 'r') as f:
        deepbindiffinfo_line = f.readline()
        deepbindiffinfo = json.loads(deepbindiffinfo_line)

    dic_size = len(deepbindiffinfo["reversed_dictionary"])
    ...
    with tf.Session(graph=g) as session:
        ...
        for index in range(len(articlelist)):
            data_index = 0
            with open(articlelist[index], 'r') as f:
                articlelist_line = f.readline()
                articleinfo = json.loads(articlelist_line)
                print(articlelist[index])
            for step in range(num_steps):
                context, target = generate_batch(articleinfo["article"], articleinfo["blockBoundaryIdx"], articleinfo["insnStartingIndices"], articleinfo["indexToCurrentInsnsStart"])
                ...
                if step % 200 == 0:
                    ...
                    print('Average loss and similarity at step {0} in {1} / {2} : {3}'.format(step, index, len(articlelist), average_loss))
                    average_loss = 0
...
def tokenEmbeddingGeneration(articlelist, reversed_dictionarydir, deepbindiffinfodir):
    embeddings = buildAndTraining(articlelist, reversed_dictionarydir, deepbindiffinfodir)
    return embeddings
