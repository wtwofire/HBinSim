...
class DataSplitter:
    ...
    def prepare_set(data_to_include, table_name, file_list, cur):
        ...
        while i < data_to_include and len(file_list) > 0:
            ...
            val_data = []
            for index in data:
                ret = cur.execute("SELECT * FROM lstm_cfg where id=?", index)
                id_ret = ret.fetchall()
                if id_ret:
                    val_data.append(index)
            cur.executemany("INSERT INTO {} VALUES (?)".format(table_name), val_data)
            i += len(val_data)
        return file_list, i
    ...
    def split_data(self, validation_dim, test_dim):
        ...
        conn = sqlite3.connect(self.db_name)
        c = conn.cursor()
        q = c.execute("SELECT id FROM lstm_cfg")
        lstm_cfg_id = q.fetchall()
        
        for index in lstm_cfg_id:
            q2 = c.execute('''SELECT project, file_name FROM functions WHERE id = ?''',index)
            lstm.append(q2.fetchall()[0])        
        #q = c.execute('''SELECT sslversion, file_name FROM functions ''')
        #data = q.fetchall()
        conn.commit()
