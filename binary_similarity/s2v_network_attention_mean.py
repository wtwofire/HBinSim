...
class NetworkLSTM:
    ...
    def Attention_Features(self, input_x, lengths):
        ...
        last_outputs = tf.squeeze(tf.nn.l2_normalize(tf.matmul(w0_tiled, flattened_inputs,
    ...
    def GCNField(self, input_x, input_adj, CONV_PARAMS, Wa, Wb, Wc, name):
        ...
        graph_embedding = tf.squeeze(tf.matmul(fi, W2_tiled, name=name + '_graph_embedding'),axis=1)
    ...
    def generateGraphClassificationNetwork(self):
        ...
        with tf.name_scope('LSTM_GCNField1'):
            self.graph_embedding_1 = tf.nn.l2_normalize(self.GCNField(self.x_1_after_blstm, self.adj_1, self.CONV_PARAMS_LSTM, self.W1_LSTM, self.W2_LSTM, self.WS1_B_LSTM, "LSTM_GCNField1"), axis=1,
                name="embedding1")

        with tf.name_scope('LSTM_GCNField2'):
            self.graph_embedding_2 = tf.nn.l2_normalize(self.GCNField(self.x_2_after_blstm, self.adj_2, self.CONV_PARAMS_LSTM, self.W1_LSTM, self.W2_LSTM,self.WS1_B_LSTM, "LSTM_GCNField2"), axis=1,
                name="embedding2")
        ...
        with tf.name_scope('graph_embedding_2'):
            ...
            self.concat_att = self.Attention_Features_HetGNN_block(self.concat_embedding_2, self.WS2_diff_feature)#[batch,1,2]
