        ...
        with tf.Graph().as_default() as g:
           ...
           # session_conf.gpu_options.per_process_gpu_memory_fraction=0.5
            ...
            for epoch in range(0, self.num_epochs):
                ...
                # ----------------------#
                n_batch=0
                for adj1_batch, nodes1_batch, adj2_batch, nodes2_batch, y_batch, len1_batch, len2_batch, nodes1_acfg_batch, nodes2_acfg_batch in p_train.async_chunker(epoch%30, self.batch_size, shuffle=True):
                ...
                for adj1_batch, nodes1_batch,  adj2_batch, nodes2_batch, y_batch, len1_batch, len2_batch, nodes1_acfg_batch, nodes2_acfg_batch in p_validation.async_chunker(0, self.batch_size):
                    ...
                    sys.stdout.write(
                        "\r\t Pairs in current batch {}, validation batch {} / {} -- {} ".format(len(len1_batch),v_batch, p_validation.num_batches, float(v_batch)/p_validation.num_batches))
                    sys.stdout.flush()
                    v_batch = v_batch+1
                ...
                if val_auc > BEST_VAL_AUC and not self.cross_val:
                    ...
                    for adj1_batch, nodes1_batch, adj2_batch, nodes2_batch, y_batch, len1_batch, len2_batch, nodes1_acfg_batch, nodes2_acfg_batch in \
                        ...
                        sys.stdout.write(
                        "\r\t Pairs in current batch {}, testing batch {} / {} -- {} ".format(len(len1_batch),t_batch, p_test.num_batches, float(t_batch)/p_test.num_batches))
                        sys.stdout.flush()
                        t_batch = t_batch+1
                    ...
                    if test_auc > BEST_TEST_AUC:
                        ...
                        print( "\tNEW BEST_TEST_AUC: {} !\n".format(BEST_TEST_AUC))
                        # write ROC raw data
                        with open(str(self.logdir) + "/best_test_roc-{}.tsv".format(BEST_TEST_AUC), "w") as the_file:
                            the_file.write("#thresholds\ttpr\tfpr\n")
                            for t, tpr, fpr in zip(test_thresholds, test_tpr, test_fpr):
                                the_file.write("{}\t{}\t{}\n".format(t, tpr, fpr))
                        # save best model
