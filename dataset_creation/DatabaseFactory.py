...
class DatabaseFactory:
    ...
    def __init__(self, db_name, root_path, info):
    ...
    def analyze_file(item):
        ...
        analyzer = RadareFunctionAnalyzer(filename, use_symbol, block_info)
        p = ThreadPool(1)
    ...
    def remove_override(self, file_list):
        conn = sqlite3.connect(self.db_name)
        cur = conn.cursor()
        q = cur.execute('''SELECT project, version, optimization, file_name FROM functions''')
        names = q.fetchall()
    ...
    def build_db(self, use_symbol):
        ...
        t_args = [(f, self.db_name, use_symbol, self.block_info) for f in file_list]

        # Start a parallel pool to analyze files
        p = Pool(processes=None, maxtasksperchild=50)
