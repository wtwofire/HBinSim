...
class FunctionSearchEngine:
    ...
    def __init__(self, db_name, table_name, limit=None):
        ...
        cur = conn.cursor()
        if limit is None:
            q = cur.execute("SELECT id, project, version, optimization, file_name, function_name FROM functions")
            res = q.fetchall()
        else:
            q = cur.execute("SELECT id, project, version, optimization, file_name, function_name FROM functions LIMIT {}".format(limit))
            res = q.fetchall()
