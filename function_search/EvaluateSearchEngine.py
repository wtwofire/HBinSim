...
class SearchEngineEvaluator:
    ...
    #
    def find_target_fcn(self, project, opt, num):
        conn = sqlite3.connect(self.db_name)
        cur = conn.cursor()
        q = cur.execute("SELECT id, project, file_name, function_name FROM functions WHERE project=? AND optimization=?", (project, opt))
        res = q.fetchall()
        ids = [i[0] for i in res]
        #version1
        #true_labels = [l[1]+"/"+l[2]+"/"+l[3] for l in res]
        #version2
        true_labels = [l[1] for l in res]
        n_ids = []
        n_true_labels = []
        num = min(num, len(ids))
        ...
        for i in range(0, num):
            ...
            q = cur.execute('SELECT id FROM functions WHERE project="{}"'.format(project))
            num=len(q.fetchall())
    ...
    #
    #
    def evaluate_precision_on_all_functions(self, project, opt):
        target_fcn_ids, true_labels = self.find_target_fcn(project, opt, 10000)
        batch = 1000
        ...
        with open("output_HBinSim_time/" + project+'_'+opt+'_'+self.tables+'_top200.json', 'w') as outfile:
...
if __name__ == '__main__':
    ...
    dbName = '/home/wangyan/experiment/HBinSim/data/HBinSim-esh-dataset-1523.db'
    table = ['HBinSim_embeddings']
    opt = ["O2"]#编辑的级别
    for x in [r"Clobberin'Time","ffmpeg",'heartbeat', "Shellshock", "Shellshock#2", "Venom", "wget", "ws-snmp"]:
        for t in table:
            for o in opt:
